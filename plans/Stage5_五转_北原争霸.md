...
**新NPC**：`Content/NPCs/NorthDesert/ZombieAllianceEnvoy.cs`

**小说依据**：僵盟是北原的神秘势力，由仙僵组成
...
|---------|------|------|
| "你是谁？" | 首次对话 | 使者自我介绍僵盟 |
| "僵盟是什么？" | 首次对话 | 解释仙僵的存在和僵盟的目的 |
| "仙僵有什么好处？" | 玩家五转 | 详细说明仙僵的增益和减益 |
| "我想成为仙僵" | 完成僵盟任务线+五转 | 触发仙僵转化流程 |
| "有没有不需要转化的任务？" | 任何条件 | 提供非仙僵任务 |
| "交易" | 任何条件 | 打开僵盟商店 |
...
| 尸道丹 | 30 | 临时获得尸道抗性 |
| 僵盟令 | 50 | 召唤僵盟使者到当前位置 |
| 不死药 | 200 | 10秒内免疫死亡（一次性） |
| 阴气瓶 | 40 | 在白天提供阴气保护（仙僵用） |

### 4.5 北原盗匪系统

**新系统**：`Common/Systems/NorthBanditSystem.cs`

**小说依据**：北原荒野中有大量盗匪，袭击商队和散修

**盗匪事件触发**：
- 在北原冰原中行走时，每分钟有10%概率触发盗匪袭击
- 护送商队任务中100%触发
- 盗匪袭击为小型入侵事件

**盗匪NPC列表**：

| NPC | 转数 | 生命 | 伤害 | 特殊能力 | 掉落 |
|-----|------|------|------|---------|------|
| 盗匪喽啰 | 三转 | 600 | 40 | 无 | 元石×2~5 |
| 盗匪头目 | 四转 | 2000 | 65 | 召唤喽啰 | 元石×10~20 |
| 冰原马贼 | 三转 | 800 | 50 | 骑马冲锋 | 马匹召唤物 |
| 盗匪术士 | 四转 | 1200 | 70 | 冰道攻击 | 冰道材料 |
| 盗匪首领 | 五转 | 5000 | 90 | 全道系攻击 | 力道蛊虫+元石×30 |
...
| 霸刀斩 | 前方扇形斩击 | 100 | 每3秒 |
| 召唤喽啰 | 召唤3个盗匪喽啰 | - | 每10秒 |
| 霸道冲锋 | 向玩家高速冲刺 | 120 | 每8秒 |
| 绝命一击（<30%血量） | 攻击+100%但防御归零 | 180 | 持续 |
...
| 条件 | 具体要求 | 检测方式 |
|------|---------|---------|
| 境界 | 玩家达到五转巅峰 | `QiRealmPlayer.GuLevel >= 5 && QiRealmPlayer.LevelStage >= 3` |
| Boss前置 | 击败冰塞川或五行大法师 | `DownBossSystem.downedBingSaiChuan \|\| downedWuXingDaFaShi` |
| 剧情前置 | 天庭前奏事件完成 | `DownBossSystem.heavenPreludeTriggered` |
| 太白云生 | 太白云生事件完成（存活或死亡） | `TaiBaiYunShengEvent.IsCompleted` |
...
| NPC | 过渡后状态 | 后续出现 |
|-----|-----------|---------|
| 黑楼兰 | 留守王庭 | Stage6宿命大战可能助战 |
| 毛里求 | 随黑楼兰 | 不再出现 |
| 冰塞川 | 根据是否被击败 | Stage6宿命大战可能助战 |
| 雪胡老祖 | 留守长生天 | 不再出现 |
| 太白云生 | 存活则在王庭；死亡则永久消失 | Stage6不再出现（已死）/提供情报（存活） |

### 8.4 关键状态继承

| 继承项 | 说明 |
|--------|------|
| 力道/宙道/五行真传 | 关键继承，影响六转装备制作 |
| 仙僵状态 | 完全继承，影响升仙条件 |
| 阵营选择 | 影响Stage6宿命大战阵营起始状态 |
| 太白云生存活标记 | 关键继承，影响Stage6情感事件 |
| 黑楼兰/冰塞川击败标记 | 影响Stage6NPC助战可能性 |
| 天庭前奏触发标记 | 影响Stage6四柱初始难度 |

### 8.5 技术实现注意事项

- 天庭浮岛在世界4个象限各生成一个，高度地表上方30~50格
- 阵营选择使用UI弹窗，选择后不可更改
- 北原区域保留，玩家可随时返回
- 天庭使者NPC为临时NPC，宿命大战开始后消失
