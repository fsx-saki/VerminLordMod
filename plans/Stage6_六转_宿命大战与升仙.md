...
| NPC名称 | 转数 | 生命 | 伤害 | 攻击方式 | 刷新频率 |
|---------|------|------|------|---------|---------|
| 智道蛊师 | 五转 | 3000 | 70 | 预判弹幕（追踪玩家移动方向） | 每5秒 |
| 星光术士 | 五转 | 2500 | 60 | 星光弹幕（从上方落下） | 每8秒 |
| 星宿守卫 | 五转 | 4000 | 80 | 近战+星光盾（免疫50%伤害） | 每10秒 |
| 星辰幻影 | 四转 | 1500 | 50 | 分身+瞬移 | 每6秒 |

**智道蛊师AI特点**：
- 预判玩家移动方向，向玩家即将到达的位置发射弹幕
- 弹幕呈星形图案（5发同时发射）
- 被击败时有30%概率自爆（范围伤害100）

**日冕浮岛小怪（=太阳柱）**：

| NPC名称 | 转数 | 生命 | 伤害 | 攻击方式 | 刷新频率 |
|---------|------|------|------|---------|---------|
| 火道蛊师 | 五转 | 2800 | 75 | 火焰弹幕+灼烧 | 每5秒 |
| 光道使者 | 五转 | 2200 | 65 | 致盲光线+光盾 | 每7秒 |
| 烈日战士 | 五转 | 3500 | 85 | 近战+火焰爆发 | 每8秒 |
| 日冕祭司 | 四转 | 1800 | 55 | 治疗+增益 | 每10秒 |

**日冕祭司AI特点**：
- 优先治疗周围受伤的NPC（每秒恢复500生命）
- 为周围NPC提供"日冕增益"（伤害+20%）
- 必须优先击杀，否则其他小怪难以击杀
...
| NPC名称 | 转数 | 生命 | 伤害 | 攻击方式 | 刷新频率 |
|---------|------|------|------|---------|---------|
| 命运蛊师 | 五转 | 2600 | 65 | 命运弹幕（随机效果） | 每5秒 |
| 运道术士 | 五转 | 2000 | 55 | 降低玩家运势+暴击 | 每7秒 |
| 宿命编织者 | 五转 | 3200 | 70 | 束缚+命运连线 | 每8秒 |
| 命运守卫 | 四转 | 2500 | 60 | 高防御+反击 | 每10秒 |

**宿命编织者AI特点**：
- 发射"命运之线"连接玩家和周围NPC
- 被连接的玩家受到的伤害会分给被连接的NPC
- 被连接的NPC受到的伤害也会分给玩家
- 必须击杀编织者才能断开连线
...
| NPC名称 | 转数 | 生命 | 伤害 | 攻击方式 | 刷新频率 |
|---------|------|------|------|---------|---------|
| 气道蛊师 | 五转 | 3000 | 70 | 气旋弹幕+气压 | 每5秒 |
| 变化道使者 | 五转 | 2800 | 75 | 变形攻击（随机形态） | 每6秒 |
| 龙卫精英 | 五转 | 4500 | 90 | 近战+龙鳞护甲 | 每8秒 |
| 龙息术士 | 四转 | 2000 | 60 | 龙息（扇形火焰） | 每7秒 |

**龙卫精英AI特点**：
- 高生命+高防御+高伤害
- 龙鳞护甲：受到伤害减少30%
- 被击败时掉落"龙鳞碎片"（制作龙系装备材料）
- 是最难击杀的小怪类型
...
| 影响项 | 正道 | 魔道 | 中立 |
|--------|------|------|------|
| 龙公战难度 | 高（龙公更强） | 中（龙公正常） | 高（无盟友） |
| 盟友NPC | 铜公+眉公+天庭蛊师×5 | 影无邪+砚石老人+影宗蛊师×5 | 无 |
| 龙公HP | 300000 | 250000 | 250000 |
| 龙公伤害 | 200 | 180 | 180 |
| 四柱难度 | 正道柱简单，其他柱正常 | 魔道柱简单，其他柱正常 | 全部正常 |
| 掉落加成 | 天庭系真传+10% | 影宗系真传+10% | 无加成 |
| NPC态度 | 正道NPC友好，魔道NPC敌对 | 魔道NPC友好，正道NPC敌对 | 全部中立 |

**阵营选择对Stage7的影响**：

| 影响项 | 正道 | 魔道 | 中立 |
|--------|------|------|------|
| 初始道主关系 | 天庭道主友好 | 影宗道主友好 | 全部中立 |
| 道主争夺 | 天庭道主让位（友好竞争） | 影宗道主协助（暗中帮助） | 需要自己挑战所有道主 |
| 仙窍入侵 | 天庭保护（入侵概率-50%） | 影宗情报（提前预警） | 无保护 |
| 尊者战 | 天庭尊者可召唤助战 | 影宗尊者提供情报 | 独自战斗 |
| 终局路线 | 偏向结局A（成为天意） | 偏向结局B（超脱） | 偏向结局C（摧毁） |

**阵营切换**：
- 阵营选择后不可更改
- 但在Stage7中，通过特殊事件可以"叛变"（代价巨大）
- 叛变条件：击败原阵营的道主 + 消耗仙元石×500 + 所有原阵营NPC好感度归零
...
| 防御 | 40 |
| 机制 | 分裂+再生+血道攻击 |
| ⚠️平衡说明 | 血劫为升仙考验，HP约为龙公的1/3，配合6波天劫弹幕构成综合挑战 |
...
## 7. 阶段过渡：六转 → 七转（升仙后原创内容）

### 7.1 过渡触发条件

| 条件 | 具体要求 | 检测方式 |
|------|---------|---------|
| 境界 | 玩家完成升仙（六转） | `DownBossSystem.hasAscended` |
| 龙公 | 击败龙公 | `DownBossSystem.downedLongGong` |
| 宿命 | 宿命蛊碎裂 | `DestinySystem.IsDestinyShattered` |
| 仙窍 | 仙窍系统激活 | `ImmortalApertureSystem.IsActive` |
...
| 变化 | 说明 |
|------|------|
| 天空裂缝 | 偶尔出现裂缝特效（宿命碎裂后果） |
| NPC对话更新 | 所有NPC增加"升仙"相关对话 |
| 天劫难度+30% | 天意不再约束天劫 |
| 运势波动增大 | 运势不再被宿命稳定 |
| 太古荒兽刷新 | 世界特定区域出现仙级敌人 |
| 仙窍可用 | 玩家可进入自己的仙窍子世界 |
...
| 继承项 | 说明 |
|--------|------|
| 所有蛊虫与装备 | 完全继承，凡蛊可升级为仙蛊 |
| 仙元系统 | 真元×100转换为仙元 |
| 宿命碎裂标记 | 永久改变NPC行为模式 |
| 阵营选择 | 影响Stage7道主争夺的起始关系 |
| 春秋蝉 | 保留但机制变化（仙级回溯） |
| 所有真传 | 完全继承，可升级为仙级真传 |
...
- 每个玩家独立升仙，不强制同步
- 已升仙玩家与未升仙玩家可共存
- 仙窍为个人子世界，其他玩家不可进入（除非邀请）
- 太古荒兽对所有玩家可见，但伤害按各自境界计算
- 道主地位为全服唯一，先到先得

### 7.6 技术实现注意事项

- 升仙为不可逆过程，确认弹窗需二次确认
- 仙元系统使用QiResourcePlayer扩展，不新建类
- 仙窍使用SubworldLibrary创建个人子世界
- 太古荒兽使用GlobalNPC修改现有敌人属性
