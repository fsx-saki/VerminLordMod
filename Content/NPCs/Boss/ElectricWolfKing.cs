...
using VerminLordMod.Content.NPCs.Enemy;
...
namespace VerminLordMod.Content.NPCs.Boss
{
	//自动加载一个小地图图标
	[AutoloadBossHead]
	public class ElectricWolfKing:ModNPC
	{
		//给二阶段另一个小地图上的boss图标
		...
		//自动实现的属性，通过使用隐藏的支持字段，其行为与变量完全相同
		public Vector2 LastFirstStageDestination { get; set; } = Vector2.Zero;

		
		//此属性使用NPC.localAI[]代替，它不会同步，但由于SpawnedMinions仅在spawn上用作标志，因此各方都会将其设置为true。
		//知道哪一方（客户端、服务器、所有）负责变量很重要，因为NPC.ai[]只有四个条目，所以明智地选择哪些东西需要同步，哪些东西不需要同步
		...
		public ref float FirstStageTimer => ref NPC.localAI[3];
		//不要尝试使用NPC.ai[4]/NPC.localAI[4]或更高的索引，它只接受0、1、2和3！
		...
		public override void SetStaticDefaults() {
			...
			NPCID.Sets.NPCBestiaryDrawModifiers drawModifiers = new NPCID.Sets.NPCBestiaryDrawModifiers() {
				CustomTexturePath = "VerminLordMod/Assets/Textures/Bestiary/MinionBoss_Preview",
		...
		public override void SetBestiary(BestiaryDatabase database, BestiaryEntry bestiaryEntry) {
			...
			bestiaryEntry.Info.AddRange(new List<IBestiaryInfoElement> {
				...
				new FlavorTextBestiaryInfoElement("E")
			});
		}
		...
		public override void ModifyNPCLoot(NPCLoot npcLoot) {
			...
			npcLoot.Add(ItemDropRule.Common(ModContent.ItemType<YuanS>(), 1, 10, 50));
			npcLoot.Add(ItemDropRule.Common(ModContent.ItemType<TenLifeGu>(), 10));
			npcLoot.Add(ItemDropRule.Common(ModContent.ItemType<ThunderWings>(), 2));
			npcLoot.Add(ItemDropRule.Common(ModContent.ItemType<ThunderBallGu>(), 2));
			npcLoot.Add(ItemDropRule.Common(ModContent.ItemType<DogControlGuItem>(), 2));
		...
		public override void OnKill() {
			...
			if (!DownBossSystem.downedMinionBoss) {
				//ModContent.GetInstance<YuanSOreSystem>().BlessWorldWithExampleOre();
			}

			// This sets downedMinionBoss to true, and if it was false before, it initiates a lantern night
			NPC.SetEventFlagCleared(ref DownBossSystem.downedMinionBoss, -1);
		...
		private void SpawnMinions() {
			...
			for (int i = 0; i < count; i++) {
				NPC minionNPC = NPC.NewNPCDirect(entitySource, (int)NPC.Center.X, (int)NPC.Center.Y, ModContent.NPCType<ElectricWolf>(), NPC.whoAmI);
				for (int j = 0; j < 100; j++) {
					Dust.NewDustDirect(minionNPC.Center, (int)NPC.Center.X, (int)NPC.Center.Y, DustID.SparkForLightDisc, NPC.whoAmI);
				}
			}
			//Main.NewText("summon");
		}
