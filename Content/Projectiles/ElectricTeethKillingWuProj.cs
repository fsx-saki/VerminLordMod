...
namespace VerminLordMod.Content.Projectiles
{
	class ElectricTeethKillingWuProj : ModProjectile
	{
		public override void SetDefaults() {
			...
			Projectile.penetrate = int.MaxValue; // 弹幕的穿透数，默认1次
			Projectile.timeLeft = int.MaxValue; // 弹幕的存活时间，它会从弹幕生成开始每次更新减1，为零时弹幕会被kill，默认3600
		...
		public override void AI() {
			...
			bool re = (timer / 100) % 2 == 0;
			
			Vector2 mousePos = (Main.MouseWorld - owner.Center).Length()<325? Main.MouseWorld:owner.Center+ Vector2.Normalize(Main.MouseWorld - owner.Center)*325;
			Vector2 playerToMouse = mousePos - owner.Center;
			Projectile.position = re ? owner.Center + playerToMouse * (timer%100/100f) : mousePos - playerToMouse * (timer%100 / 100f);
			Projectile.rotation = playerToMouse.ToRotation();
		...
		public override void OnHitNPC(NPC target, NPC.HitInfo hit, int damageDone) {
			for(int i = 0; i < 5; i++)
			Dust.NewDustDirect(target.position, target.width, target.height, DustID.Electric).velocity *= 0.7f;
		}
	}
}
