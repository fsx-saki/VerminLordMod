...
namespace VerminLordMod.Content.Projectiles
{
	class SawtoothGoldenWuProj : ModProjectile
	{
		...
		public override void AI() {
			...
			Vector2 mousePos = (Main.MouseWorld - owner.Center).Length()<275? Main.MouseWorld:owner.Center+ Vector2.Normalize(Main.MouseWorld - owner.Center)*275;
		...
		public override void OnHitNPC(NPC target, NPC.HitInfo hit, int damageDone) {
			for(int i = 0; i < 5; i++)
			Dust.NewDustDirect(target.position, target.width, target.height, DustID.Gold).velocity *= 0.7f;
		}
	}
}
