...
namespace VerminLordMod.Content.Projectiles
{
	class ShuangXiProj : ModProjectile
	{
		public override void SetDefaults() {
			...
			Projectile.timeLeft = 120; // 弹幕的存活时间，它会从弹幕生成开始每次更新减1，为零时弹幕会被kill，默认3600
		...
		public override void PostAI() {
			var d1 = Dust.NewDustDirect(Projectile.position, Projectile.width, Projectile.height, DustID.Ice); // Makes the projectile emit dust.
			var d2 = Dust.NewDustDirect(Projectile.position, Projectile.width, Projectile.height, DustID.Ice); // Makes the projectile emit dust.
			var d3 = Dust.NewDustDirect(Projectile.position, Projectile.width, Projectile.height, DustID.Ice); // Makes the projectile emit dust.
			d1.velocity *= 0.3f;
			d2.velocity *= 0.3f;
			d3.velocity *= 0.3f;
			d1.noGravity = true;
