...
{
	class BloodSkullProj : ModProjectile
	{
		...
		public override void SetDefaults() {
			...
			Projectile.scale = 1.1f; // 弹幕缩放倍率，会影响碰撞箱大小，默认1f
			...
			Projectile.alpha = 40; // 弹幕的透明度，0 ~ 255，0是完全不透明（int）
								  // Projectile.Opacity = 1; // 弹幕的不透明度，0 ~ 1，0是完全透明，1是完全不透明(float)，用哪个你们自己挑，这两是互相影响的
			Projectile.friendly = false; // 弹幕是否攻击敌方，默认false
			Projectile.hostile = true; // 弹幕是否攻击友方和城镇NPC，默认false
		...
		public override void AI() {
			...
			for (int i = 0; i < 10; i++) {
				Dust.NewDustDirect(Projectile.position, Projectile.width, Projectile.height, DustID.Blood).noGravity = true;
			}
		...
		public override void OnHitNPC(NPC target, NPC.HitInfo hit, int damageDone) {
			...
			if (Main.LocalPlayer != null&&target.life<=0) {
				BloodSkullGu bloodSkullGu = null;
				if (Main.LocalPlayer.HeldItem.type == ModContent.ItemType<BloodSkullGu>()) {
					bloodSkullGu= Main.LocalPlayer.HeldItem.ModItem as BloodSkullGu;
				}
				else {
					foreach (var item in Main.LocalPlayer.inventory) {
						if (item.type == ModContent.ItemType<BloodSkullGu>()) {
							bloodSkullGu = item.ModItem as BloodSkullGu;
							break;
						}
					}
				}
				if (bloodSkullGu == null)return;
				bloodSkullGu.deads += 1;
				bloodSkullGu.SetDefaults();
				//Main.NewText(bloodSkullGu.deads);
				for (int i = 0; i <= 50; i++)
					Dust.NewDustDirect(target.position, target.width, target.height, DustID.RedMoss).noGravity = false;
