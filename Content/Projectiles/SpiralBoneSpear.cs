...
namespace VerminLordMod.Content.Projectiles
{
	class SpiralBoneSpear : ModProjectile
	{
		...
		public override void AI() {
			...
			Vector2 targetPos = mousePos - player.Center;
			targetPos = targetPos.Length() < Vector2.Normalize(targetPos).Length() * 160 ? targetPos : Vector2.Normalize(targetPos) * 160;

			int opt = frametime / 36 % 2 == 0 ? 1 : -1;
			float x = opt == 1 ? frametime % 36 : 36 - frametime % 36;
			
			Projectile.position = player.position + targetPos + new Vector2(x-18, ((float)Math.Sqrt(Math.Abs(324 - Math.Pow(frametime % 36 - 18, 2)))) * opt-18);
			Projectile.rotation=targetPos.ToRotation()+0.5f*(float)Math.PI;
			frametime++;
		}
	}
}
