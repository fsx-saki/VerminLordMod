...
namespace VerminLordMod.Content.Projectiles
{
	public class YanTongBeam : ModProjectile
	{
		...
		private const float PiBeamDivisor = MathHelper.Pi / YanTongHoldout.NumBeams;
		...
		public override void AI() {
			...
			if (Projectile.type != ModContent.ProjectileType<YanTongBeam>() || !hostPrism.active || hostPrism.type != ModContent.ProjectileType<YanTongHoldout>()) {
				Projectile.Kill();
				return;
			}
			...
			float chargeRatio = MathHelper.Clamp(hostPrism.ai[0] / YanTongHoldout.MaxCharge, 0f, 1f);
			...
			Projectile.friendly = hostPrism.ai[0] > YanTongHoldout.DamageStart;

			// This offset is used to make each individual beam orient differently based on its Beam ID.
			float beamIdOffset = BeamID - YanTongHoldout.NumBeams / 2f + 0.5f;
			float beamSpread;
			float spinRate;
			...
			float deviationAngle = (hostPrism.ai[0] + beamIdOffset * spinRate) / (spinRate * YanTongHoldout.NumBeams) * MathHelper.TwoPi;
		...
		private Color GetOuterBeamColor() {
			...
			float hue = (BeamID / YanTongHoldout.NumBeams) % BeamHueVariance + BeamColorHue;
		...
		private void ProduceBeamDust(Color beamColor) {
			...
			Dust dust = Dust.NewDustDirect(endPosition, 0, 0, DustID.FlameBurst, velocity.X, velocity.Y, 0, beamColor, scale);
