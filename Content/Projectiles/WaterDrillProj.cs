		...
		public override void AI() {
			...
			if (Main.myPlayer == Projectile.owner) {
				...
				if (player.channel) {
					...
					Projectile.velocity = holdoutOffset*2;
				}
				else {
					Projectile.Kill();
				}
			...
			// Spawning dust
				Dust dust = Dust.NewDustDirect(Projectile.position + Projectile.velocity * Main.rand.Next(6, 10) * 0.15f, Projectile.width, Projectile.height, DustID.Water);
				dust.position.X -= 4f;
				dust.noGravity = true;
