...
namespace VerminLordMod.Content.Tiles
{
	...
	// YuanSOreSystem contains code related to spawning ExampleOre. It contains both spawning ore during world generation, seen in ModifyWorldGenTasks, and spawning ore after defeating a boss, seen in BlessWorldWithExampleOre and MinionBossBody.OnKill.
	public class YuanSOreSystem : ModSystem
	{
		...
		public void BlessWorldWithExampleOre() {
			...
			ThreadPool.QueueUserWorkItem(_ => {
				...
				for (int iteration = 0; iteration < splotches; iteration++) {
					...
					WorldGen.OreRunner(i, j, WorldGen.genRand.Next(5, 9), WorldGen.genRand.Next(5, 9), (ushort)ModContent.TileType<YuanSOre>());
				}
			});
		}
	...
	public class ExampleOrePass : GenPass
	{
		...
		protected override void ApplyPass(GenerationProgress progress, GameConfiguration configuration) {
			...
			progress.Message = YuanSOreSystem.ExampleOrePassMessage.Value;

			// Ores are quite simple, we simply use a for loop and the WorldGen.TileRunner to place splotches of the specified Tile in the world.
			// "6E-05" is "scientific notation". It simply means 0.00006 but in some ways is easier to read.
			for (int k = 0; k < (int)(Main.maxTilesX * Main.maxTilesY * 1E-04); k++) {
				...
				WorldGen.TileRunner(x, y, WorldGen.genRand.Next(6, 14), WorldGen.genRand.Next(2, 6), ModContent.TileType<YuanSOre>());
