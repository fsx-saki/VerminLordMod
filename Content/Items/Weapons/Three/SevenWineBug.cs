...
{
	class SevenWineBugWeapon : PractiseWeapon
	{
		...
		public override bool CanUseItem(Player player) {
			...
			//
			if (qiPlayer.qiLevel != 3) {
				Text.ShowTextRed(player, $"您不是三转蛊师,该蛊不能为你提升真元恢复速度。");
				return false;
			}
			if (qiPlayer.hasSevenWineBug == true) {
				Text.ShowTextRed(player, $"您已经使用过七香酒虫,该蛊不能为你提升真元恢复速度。");
				return false;
			}
		...
		public override bool? UseItem(Player player) {
			...
			qiPlayer.hasSevenWineBug = true;
			qiPlayer.extraQiRegen += 7;
			qiPlayer.qiCurrent -= qiCost;
			//player.GetDamage<DamageClass.Melee>()+=0.01f
			Text.ShowTextRed(player,$"使用七香酒虫，真元恢复永久提升7点。");
			return true;
		...
		public override void AddRecipes() {// 这个括号里有一个默认的参数1，你可以填任何大于0的整数。它代表了这个合成表会生成多少个物品
			...
			.AddIngredient(ModContent.GetInstance<Consumables.FourFlavorWineBug>(), 2)
			...
			.AddIngredient(ModContent.GetInstance<YuanS>(), 500)
...
// 合成材料，需要10个泥土块
// 如果你写两种相同的材料的话，它们会分别显示而不是合并成一个
.AddIngredient(ModContent.GetInstance<InvitingMoonGu>(), 1)
.AddIngredient(ItemID.LifeforcePotion, 1)
.AddIngredient(ItemID.InfernoPotion, 1)
.AddIngredient(ItemID.WarmthPotion, 1)
.AddIngredient(ItemID.TitanPotion, 1)
