	...
	class IceCrystalGu : IceSnowWeapon
	{
		...
		public override bool? UseItem(Player player) {
			if (player.altFunctionUse == 2) {
				if (controlRate >= 100 && player.HasBuff(ModContent.BuffType<IceBladeStorm1>())) {
					player.ClearBuff(ModContent.BuffType<IceBladeStorm1>());
					player.AddBuff(ModContent.BuffType<IceBladeStorm2>(), 300);
