...
namespace VerminLordMod.Content.Items.Weapons.Four
{
	class NineWineBugWeapon : PractiseWeapon
	{
		...
		public override bool CanUseItem(Player player) {
			...
			//
			if (qiPlayer.qiLevel != 4) {
				Text.ShowTextRed(player, $"您不是四转蛊师,该蛊不能为你提升真元恢复速度。");
				return false;
			}
			if (qiPlayer.hasNineWineBug == true) {
				Text.ShowTextRed(player, $"您已经使用过九眼酒虫,该蛊不能为你提升真元恢复速度。");
				return false;
			}
		...
		public override bool? UseItem(Player player) {
			...
			qiPlayer.hasNineWineBug = true;
			qiPlayer.extraQiRegen += 9;
			qiPlayer.qiCurrent -= qiCost;
			//player.GetDamage<DamageClass.Melee>()+=0.01f
			Text.ShowTextRed(player,$"使用九眼酒虫，真元恢复永久提升9点。");
			return true;
		...
		public override void AddRecipes() {// 这个括号里有一个默认的参数1，你可以填任何大于0的整数。它代表了这个合成表会生成多少个物品
			...
			.AddIngredient(ModContent.GetInstance<Weapons.Three.SevenWineBugWeapon>(), 2)
			...
			.AddIngredient(ModContent.GetInstance<YuanS>(), 5000)
			...
			.AddIngredient(ItemID.DestroyerTrophy, 1)
			.AddIngredient(ItemID.SkeletronPrimeTrophy, 1)
			.AddIngredient(ItemID.SpazmatismTrophy, 1)
			.AddIngredient(ModContent.GetInstance<YuanS>(), 5000)
			.AddOnCraftCallback(RefineRecipeCallbacks.IfFailed)
			// 使用ModContent.ItemType<xxx>()来获取你的物品的type
			//.AddIngredient(ModContent.ItemType<SkirtSword>(), 1)
