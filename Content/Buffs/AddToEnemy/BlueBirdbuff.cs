...
namespace VerminLordMod.Content.Buffs.AddToEnemy
{
	class BlueBirdbuff : ModBuff
	{
		...
		public override void Update(NPC npc, ref int buffIndex) {
			var d = Dust.NewDustDirect(npc.position, npc.width, npc.height, DustID.Ice);
			d.velocity *= 0.1f;
			d.noGravity = true;
			...
			if (npc.life <= 0) {
				for (int i = 0; i < 100; i++) {
					var d1 = Dust.NewDustDirect(npc.position, npc.width, npc.height, DustID.Ice);
					d1.velocity *= 0.5f;
					d1.noGravity = true;
				}
			}
