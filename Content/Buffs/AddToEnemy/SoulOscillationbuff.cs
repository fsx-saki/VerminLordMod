...
{
	class SoulOscillationbuff : ModBuff
	{
		...
		public override void Update(NPC npc, ref int buffIndex) {
			var dust = Dust.NewDustDirect(npc.position, npc.width, npc.height, DustID.GreenFairy);

			var random = new Random();
			dust.color = new Color(random.Next(0, 255), random.Next(0, 255), random.Next(0, 255));
			float r = random.Next(-4, 4) * random.NextSingle();
			npc.velocity *= Math.Abs(r) <= 0.1 ? 1 : r;
		}
